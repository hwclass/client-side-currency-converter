    ...
    return {
      ...
      */
      newDanishKronInputValue : function (data) {
        var self = this;
        var notifiedData = data.value;
        this.notifyAjax('getNewDanishKronToLiraConversion', {
          url : sandbox.use('config').API.URL.CONVERT + sandbox.use('config').API.ENDPOINT + '?access_key=' + sandbox.use('config').API.ACCESS_KEY + '&from=' + sandbox.use('config').CURRENCY.KRON + '&to=' + sandbox.use('config').CURRENCY.LIRA + '&amount=' + notifiedData,
          type : 'jsonp',
          success : function (data) {
            if (!isNaN(notifiedData) && !!data.success) {
              self.$liraInput[0].value = sandbox.use('numeral')(parseInt(notifiedData) * data['quotes']['DKKTRY']).format('0,0.00[0]');
            }
          },
          error : function (e) {
            console.log(e);
