  ...
  Core.register('sterlinInput', function (sandbox) {
    ...
    return {
      ...
      newDollarInputValue : function (data) {
        ...
        sandbox.use('$').ajax({
          type: 'GET',
          url : sandbox.use('config').API.URL.CONVERT + '?access_key=' + sandbox.use('config').API.ACCESS_KEY + '&from=' + sandbox.use('config').CURRENCY.DOLLAR + '&to=' + sandbox.use('config').CURRENCY.STERLIN + '&amount=' + notifiedData,
          crossDomain: true,
          dataType: 'jsonp',
      ...
      newEuroInputValue : function (data) {
        ...
        sandbox.use('$').ajax({
          type: 'GET',
          url : sandbox.use('config').API.URL.CONVERT + '?access_key=' + sandbox.use('config').API.ACCESS_KEY + '&from=' + sandbox.use('config').CURRENCY.EURO + '&to=' + sandbox.use('config').CURRENCY.STERLIN + '&amount=' + notifiedData,
          crossDomain: true,
          dataType: 'jsonp',
          success: function(data) {
            if (!isNaN(notifiedData)) {
      ...
      newYenInputValue : function (data) {
        ...
        sandbox.use('$').ajax({
          type: 'GET',
          url : sandbox.use('config').API.URL.CONVERT + '?access_key=' + sandbox.use('config').API.ACCESS_KEY + '&from=' + sandbox.use('config').CURRENCY.YEN + '&to=' + sandbox.use('config').CURRENCY.STERLIN + '&amount=' + notifiedData,
          crossDomain: true,
          dataType: 'jsonp',
          success: function(data) {
            if (!isNaN(notifiedData)) {
      ...
      newLiraInputValue : function (data) {
        ...
        sandbox.use('$').ajax({
          type: 'GET',
          url : sandbox.use('config').API.URL.CONVERT + '?access_key=' + sandbox.use('config').API.ACCESS_KEY + '&from=' + sandbox.use('config').CURRENCY.LIRA + '&to=' + sandbox.use('config').CURRENCY.STERLIN + '&amount=' + notifiedData,
          crossDomain: true,
          dataType: 'jsonp',
          success: function(data) {
            if (!isNaN(notifiedData)) {
              self.$sterlinInput[0].value = sandbox.use('numeral')(parseInt(notifiedData) * data.results['TRY_GBP'].val).format("0,0.00[0]");
            }
          }
        });
      },
